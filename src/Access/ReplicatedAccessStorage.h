...
private:
    ...
    void refreshEntityNoLock(const zkutil::ZooKeeperPtr& zookeeper, const UUID& id, Notifications& notifications) TSA_REQUIRES(mutex);

    void setEntityNoLock(const UUID& id, const AccessEntityPtr& entity, Notifications& notifications) TSA_REQUIRES(mutex);
    void removeEntityNoLock(const UUID& id, Notifications& notifications) TSA_REQUIRES(mutex);
