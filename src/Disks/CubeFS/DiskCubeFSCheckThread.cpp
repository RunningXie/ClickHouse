...
DiskCubeFSCheckThread::DiskCubeFSCheckThread(DiskCubeFS * disk_, ContextPtr context_, UInt64 local_disk_check_period_ms)
    : WithContext(context_)
    , disk(std::move(disk_))
    , check_period_ms(local_disk_check_period_ms)
    , log(&Poco::Logger::get(fmt::format("DiskCubeFSCheckThread({})", disk->getName())))
{
    task = getContext()->getSchedulePool().createTask(log->name(), [this] { run(); });
}

void DiskCubeFSCheckThread::startup()
{
    need_stop = false;
    retry = 0;
...
void DiskCubeFSCheckThread::run()
{
    if (need_stop)
        return;
...
void DiskCubeFSCheckThread::shutdown()
{
    need_stop = true;
    task->deactivate();
    LOG_TRACE(log, "DiskCubeFSCheck thread finished");
}
