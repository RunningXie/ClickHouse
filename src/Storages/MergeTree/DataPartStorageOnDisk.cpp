    ...
    Poco::Logger * log)
{
    ...
    if (!has_delete_prefix)
    {
        ...
        if (disk->supportDataSharing() && !disk->exists(from))
        {
            LOG_INFO(log, "Directory {} (to which part must be renamed before removing) already not exists in disk {}. "
                    "Most likely it is removed by other replicas.", fullPath(disk, from), disk->getName());
            return;
        }
...
bool DataPartStorageOnDisk::supportDataSharing() const
{
    return volume->getDisk()->supportDataSharing();
}
