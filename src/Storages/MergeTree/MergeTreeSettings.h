...
#define LIST_OF_MERGE_TREE_SETTINGS(M) \
    ...
    M(UInt64, merge_tree_clear_old_broken_detached_parts_ttl_timeout_seconds, 1ULL * 3600 * 24, "Remove old broken detached parts in the background if they remained intouched for a specified by this setting period of time.", 0) \
    M(UInt64, merge_tree_enable_clear_old_broken_detached, true, "Enable clearing old broken detached parts operation in background.", 0) \
    \
    /** Inserts settings. */ \
