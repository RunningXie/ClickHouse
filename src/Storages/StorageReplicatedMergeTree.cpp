    ...
    , replicated_sends_throttler(std::make_shared<Throttler>(getSettings()->max_replicated_sends_network_bandwidth, getContext()->getReplicatedSendsThrottler()))
{
    ...
    MergeTreeData::init();
    reader = make_shared<MergeTreeDataSelectExecutor>(*this);
    writer = make_shared<MergeTreeDataWriter>(*this);
    merger_mutator = make_shared<MergeTreeDataMergerMutator>(*this,
        getContext()->getSettingsRef().background_merges_mutations_concurrency_ratio *
        getContext()->getSettingsRef().background_pool_size);
    merge_strategy_picker = make_shared<MergeStrategyPicker>(*this);
    queue = make_shared<ReplicatedMergeTreeQueue>(*this, merge_strategy_picker);
    fetcher = make_shared< DataPartsExchange::Fetcher>(*this);
    cleanup_thread = make_shared<ReplicatedMergeTreeCleanupThread>(*this);
    part_check_thread = make_shared<ReplicatedMergeTreePartCheckThread>(*this);
    restarting_thread = make_shared<ReplicatedMergeTreeRestartingThread>(*this);
    part_moves_between_shards_orchestrator = make_shared<PartMovesBetweenShardsOrchestrator>(*this);
...
void StorageReplicatedMergeTree::drop()
{
    ...
    dropAllData(needRemoveSharedDataParts());
}
...
bool StorageReplicatedMergeTree::needRemoveSharedDataParts()
{
    ...
    int retry = 0;
    String root_path = zookeeper_path + "/replicas";
    while (retry++ < ZK_MAX_RETRY_COUNT)
    {
        Coordination::Error error = zookeeper->tryGetChildren(root_path, replicas);
        if (error == Coordination::Error::ZNONODE) { return true; }
    ...
    const DataPartPtr & part)
{
    ...
    while (true)
    {
        ...
        {
            LOG_INFO(log, "Do not commit if the part is obsolete, we have just briefly checked its checksums");
            return {};
        }
...
bool StorageReplicatedMergeTree::executeFetch(LogEntry & entry)
{
    ...
    try
    {
        if (replica.empty())
        {
            ...
            if (replica.empty())
            {
                String unactive_replica = findReplicaHavingCoveringPart(entry, false);
...
String StorageReplicatedMergeTree::findReplicaHavingCoveringPart(LogEntry & entry, bool active)
{
    ...
    for (const String & replica : replicas)
    {
        ...
        {
            LOG_TRACE(log, "unactive replica: {}", replica);
            continue;
        }
    ...
    const String & source_replica_path, bool to_detached, size_t quorum, zkutil::ZooKeeper::Ptr zookeeper_)
{
    ...
    auto existed_part = getActiveContainingPart(part_info);
    if (!existed_part)
    {
        auto disks = getDisks();
        ...
        for (auto disk : disks)
        {
            if (disk->supportDataSharing() && disk->exists(part_relative_path))
            {
                ...
                Transaction transaction(*this);
                transaction.addPart(fetched_part);
    ...
    zkutil::ZooKeeperPtr & zookeeper, String part_name, LogEntry & entry, bool detach, bool throw_if_noop)
{
    ...
    while (true)
    {
        ...
        LOG_TRACE(log, "A new log entry commit DROP RANGE for part {}.", part_name);
        return true;
    }
}
...
DistributeLockGuardPtr StorageReplicatedMergeTree::getDistributeLockGuardImpl(const zkutil::ZooKeeperPtr& zookeeper, const String& disk_name,
...
DistributeLockGuardPtr StorageReplicatedMergeTree::getDistributeLockGuardInInit(
    const String& disk_name,
    const String& part_name,
    const String& value) const {
    return getDistributeLockGuardImpl(getZooKeeper(), disk_name, part_name, value);
}
    ...
    const String& value) const
{
    ...
    return getDistributeLockGuardImpl(zookeeper, disk_name, part_name, value);
}
...
Strings StorageReplicatedMergeTree::getChildrenFromZKWithRetry(String path, int retry_time) const
{
    auto zookeeper = getZooKeeper();
    ...
    while(times++ < retry_time)
    {
        try
        {
            auto parts = zookeeper->getChildren(path);
            return parts;
        }
        catch(zkutil::KeeperException e)
        {
            ...
            else if (e.code == Coordination::Error::ZBADVERSION)
                LOG_DEBUG(log, "get parts from zookeeper failed, retry it later. exception : {}.", e.what());
            else
                e.rethrow();
        }
    }
...
std::unordered_set<String> StorageReplicatedMergeTree::getMissingPartsFromZK() const 
{
    std::unordered_set<String> local_parts;
    std::unordered_set<String> other_parts;

    Strings replicas = getChildrenFromZKWithRetry(zookeeper_path + "/replicas");
    for (auto& replica : replicas)
    {
        Strings parts = getChildrenFromZKWithRetry(fs::path(zookeeper_path) / "replicas" / replica / "parts");
        if (replica != replica_name)
        {
            other_parts.insert(parts.begin(), parts.end());
        }
        else
        {
            local_parts.insert(parts.begin(), parts.end());
        }
    }
    for (auto& local_part : local_parts)
    {
        if (other_parts.contains(local_part))
            other_parts.erase(local_part);
    }
    return other_parts;
}
